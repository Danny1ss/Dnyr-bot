...
ARQ_API_KEY = "AAGRhkw8uwRzE0d6IIpyK-L0p-LCpc8tJ1k"
ARQ_API_URL = "http//t.me/d_nyr"
...
"""
         r.get(f'{m.chat.id}:mute:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockJoin:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockChannels:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockEdit:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockEditM:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockVoice:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockVideo:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockNot:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockPhoto:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockStickers:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockAnimations:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockFiles:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockPersian:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockUrls:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockHashtags:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockMessages:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockTags:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockBots:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockSpam:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockInline:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockForward:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockAudios:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockaddContacts:{Dev_dnyr}')
         r.get(f'{m.chat.id}:lockSHTM:{Dev_dnyr}')
"""
...
async def on_zbi(c: Client, m: Message):
    name = r.get(f"{Dev_dnyr}:BotName") if r.get(f"{Dev_dnyr}:BotName") else "ليو"
    text = m.text
    if text.startswith(f"{name} "):
        text = text.replace(f"{name} ", "")
    if r.get(f"{m.chat.id}:Custom:{m.chat.id}{Dev_dnyr}&text={text}"):
        text = r.get(f"{m.chat.id}:Custom:{m.chat.id}{Dev_dnyr}&text={text}")
    if r.get(f"Custom:{Dev_dnyr}&text={text}"):
        text = r.get(f"Custom:{Dev_dnyr}&text={text}")

    if r.get(f"inDontCheck:{Dev_dnyr}"):
    ...
    if (
        ...
        if r.get(f"forceChannel:{Dev_dnyr}") and (
            not r.get(f"disableSubscribe:{Dev_dnyr}")
        ):
            username = r.get(f"forceChannel:{Dev_dnyr}").replace("@", "")
            not_member = False
            try:
                ...
                r.set(f"inDontCheck:{Dev_dnyr}", 1, ex=10)
            ...
            if not_member:
                ...
                r.set(f"inDontCheck:{Dev_dnyr}", ex=10)
                return m.stop_propagation()
            else:
                return m.continue_propagation()
...
def guardLocksResponse(c, m):
    k = r.get(f"{Dev_dnyr}:botkey")
    channel = (
        r.get(f"{Dev_dnyr}:BotChannel") if r.get(f"{Dev_dnyr}:BotChannel") else "YQYQY6"
    )
...
def guardResponseFunction2(c, m, k, channel):
    if not r.get(f"{m.chat.id}:enable:{Dev_dnyr}"):
        return
    warner = """
「 {} 」
{} ممنوع {}
...
"""
    ...
    if (
        ...
        warn = True
        reason = "التعديل"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
            f"{Dev_dnyr}:inWARN:{m.from_user.id}{m.chat.id}"
        ):
            r.set(f"{Dev_dnyr}:inWARN:{m.from_user.id}{m.chat.id}", 1, ex=60)
            return m.reply(
...
"""
    ...
    if r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}") and m.service:
        m.delete()
    ...
    if (
        ...
        for me in m.new_chat_members:
            if not me.id == m.from_user.id:
                ...
                reason = "تضيف حد هنا"
                m.delete()
                if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}"):
                    return m.reply(
                        warner.format(mention, k, reason), disable_web_page_preview=True
                    )
    ...
    if m.media:
        ...
        idd = file_id[-6:]
        if r.get(f"{idd}:NotAllow:{m.chat.id}{Dev_dnyr}"):
            if not admin_pls(id, m.chat.id):
                return m.delete()

    if m.text and r.smembers(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"):
        if not admin_pls(id, m.chat.id):
            for word in r.smembers(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"):
                if word in m.text:
                    return m.delete()

    if r.get(f"{id}:mute:{m.chat.id}{Dev_dnyr}") or r.get(f"{id}:mute:{Dev_dnyr}"):
        return False

    if r.get(f"{m.chat.id}:mute:{Dev_dnyr}") and not admin_pls(id, m.chat.id):
        m.delete()
        return False
    ...
    if r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}") and m.new_chat_members:
        for mem in m.new_chat_members:
            if mem.is_bot:
                return m.chat.ban_member(mem.id)

    if r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}") and m.new_chat_members:
        for mem in m.new_chat_members:
    ...
    if r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}") and m.sender_chat:
        if not m.sender_chat.id == m.chat.id:
            m.chat.ban_member(m.sender_chat.id)
            return False

    if r.get(f"{m.chat.id}:lockSpam:{Dev_dnyr}"):
        if not r.get(f"{id}in_spam:{m.chat.id}{Dev_dnyr}"):
            r.set(f"{id}in_spam:{m.chat.id}{Dev_dnyr}", 1, ex=10)
        else:
            if int(r.get(f"{id}in_spam:{m.chat.id}{Dev_dnyr}")) == 10:
                if m.from_user:
                    r.set(f"{id}:mute:{m.chat.id}{Dev_dnyr}", 1)
                    r.sadd(f"{m.chat.id}:listMUTE:{Dev_dnyr}", id)
                    r.delete(f"{id}in_spam:{m.chat.id}{Dev_dnyr}")
                    return m.reply(
                ...
                get = int(r.get(f"{id}in_spam:{m.chat.id}{Dev_dnyr}"))
                r.set(f"{id}in_spam:{m.chat.id}{Dev_dnyr}", get + 1, ex=10)

    if r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}") and m.via_bot:
        m.delete()
        warn = True
        reason = "ترسل انلاين"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}") and m.forward_date:
        m.delete()
        warn = True
        reason = "ترسل توجيه"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    """
  if r.get(f'{m.chat.id}:lockForward:{Dev_dnyr}') and m.forward_from_chat:
     m.delete()
     warn = True
     reason = 'ترسل توجيه'
     if not r.get(f'{m.chat.id}:disableWarn:{Dev_dnyr}') and not r.get(f'{Dev_dnyr}:inWARN:{m.from_user.id}{m.chat.id}'):
        r.set(f'{Dev_dnyr}:inWARN:{m.from_user.id}{m.chat.id}',1,ex=60)
        return m.reply(warner.format(mention,k,reason),disable_web_page_preview=True)
  """

    if r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}") and m.audio:
        m.delete()
        warn = True
        reason = "ترسل صوت"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}") and m.video:
        m.delete()
        warn = True
    ...
    if r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}") and m.photo:
        m.delete()
        warn = True
        reason = "ترسل صور"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}") and m.sticker:
        m.delete()
        warn = True
        reason = "ترسل ملصقات"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}") and m.animation:
        m.delete()
        warn = True
        reason = "ترسل متحركات"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}") and m.document:
        m.delete()
        warn = True
        reason = "ترسل ملفات"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}") and m.text:
    ...
    if r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}") and m.caption:
    ...
    if r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}") and m.text and len(m.text) > 150:
        m.delete()
        warn = True
    ...
    if r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}") and m.voice:
        m.delete()
        warn = True
        reason = "ترسل فويس"
        if not r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}") and not r.get(
    ...
    if r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}") and (m.caption or m.text):
        if m.caption:
            txt = m.caption
        if m.text:
            txt = m.text
    ...
    """
  if r.get(f'{m.chat.id}:lockKFR:{Dev_dnyr}') and (m.caption or m.text):
     if m.caption:
  ...
  """
    ...
    if r.get(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}") and m.new_chat_members:
...
""".format(m.from_user.mention, k)
    ...
    if r.get(f"{m.chat.id}:enableVerify:{Dev_dnyr}") and m.new_chat_members:
        for me in m.new_chat_members:
    ...
    if m.media and r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}"):
        print("nsfw scanner")
        if not admin_pls(id, m.chat.id):
            if m.sticker:
...
def antiPersian(c, m):
    if r.get(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}"):
        k = r.get(f"{Dev_dnyr}:botkey")
...
def guardCommands(c, m, k, channel):
    if not r.get(f"{m.chat.id}:enable:{Dev_dnyr}"):
        return False
    if r.get(f"{m.chat.id}:mute:{Dev_dnyr}") and not admin_pls(
        m.from_user.id, m.chat.id
    ):
        return False
    if r.get(f"{m.from_user.id}:mute:{m.chat.id}{Dev_dnyr}"):
        return False
    if r.get(f"{m.from_user.id}:mute:{Dev_dnyr}"):
        return False
    if r.get(f"{m.chat.id}:addCustom:{m.from_user.id}{Dev_dnyr}"):
        return False
    if r.get(f"{m.chat.id}addCustomG:{m.from_user.id}{Dev_dnyr}"):
        return False
    if r.get(f"{m.chat.id}:delCustom:{m.from_user.id}{Dev_dnyr}") or r.get(
        f"{m.chat.id}:delCustomG:{m.from_user.id}{Dev_dnyr}"
    ):
        return False
    text = m.text
    name = r.get(f"{Dev_dnyr}:BotName") if r.get(f"{Dev_dnyr}:BotName") else "ليو"
...
"""
    ...
    if text == "الاعدادات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            x1 = "مقفول" if r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}") else "مفتوح"
            x2 = "مقفول" if r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}") else "مفتوح"
            x3 = "مقفول" if r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}") else "مفتوح"
            x4 = "مقفول" if r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}") else "مفتوح"
            x5 = "مقفول" if r.get(f"{m.chat.id}:mute:{Dev_dnyr}") else "مفتوح"
            x6 = "مقفول" if r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}") else "مفتوح"
            x7 = "مقفول" if r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}") else "مفتوح"
            x8 = "مقفول" if r.get(f"{m.chat.id}:lockHashtags:{Dev_dnyr}") else "مفتوح"
            x9 = "مقفول" if r.get(f"{m.chat.id}:lockEdit:{Dev_dnyr}") else "مفتوح"
            x10 = "مقفول" if r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}") else "مفتوح"
            x11 = "مقفول" if r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}") else "مفتوح"
            x12 = (
                "مقفول" if r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}") else "مفتوح"
            )
            x13 = "مقفول" if r.get(f"{m.chat.id}:lockUrls:{Dev_dnyr}") else "مفتوح"
            x14 = "مقفول" if r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}") else "مفتوح"
            x15 = "مقفول" if r.get(f"{m.chat.id}:lockTags:{Dev_dnyr}") else "مفتوح"
            x16 = "مقفول" if r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}") else "مفتوح"
            x17 = (
                "مقفول" if r.get(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}") else "مفتوح"
            )
            x18 = "مقفول" if r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}") else "مفتوح"
            x19 = "مقفول" if r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}") else "مفتوح"
            x20 = "مقفول" if r.get(f"{m.chat.id}:lockSpam:{Dev_dnyr}") else "مفتوح"
            x21 = "مقفول" if r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}") else "مفتوح"
            x22 = "مقفول" if r.get(f"{m.chat.id}:lockEditM:{Dev_dnyr}") else "مفتوح"
            x23 = "مقفول" if r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}") else "مفتوح"
            x24 = "مقفول" if r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}") else "مفتوح"
            x25 = (
                "مقفول" if r.get(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}") else "مفتوح"
            )
            x26 = "مقفول" if r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}") else "مفتوح"
            return m.reply(f"""
اعدادات المجمشعة :
...
~ @{channel}""")
    ...
    if text == "القوانين":
        if r.get(f"{m.chat.id}:CustomRules:{Dev_dnyr}"):
            rules = r.get(f"{m.chat.id}:CustomRules:{Dev_dnyr}")
        else:
...
""")
    ...
    if text == "اطردني":
        if r.get(f"{m.chat.id}:enableKickMe:{Dev_dnyr}"):
    ...
    if text == "الرابط":
        if not r.get(f"{m.chat.id}:disableLINK:{Dev_dnyr}"):
            link = c.get_chat(m.chat.id).invite_link
    ...
    if text.startswith("@all"):
        if not mod_pls(m.from_user.id, m.chat.id):
            return m.reply(f"{k} هذا الامر يخص ( المدير وفوق ) بس")
        if r.get(f"{m.chat.id}:disableALL:{Dev_dnyr}"):
            return m.reply("المنشن معطل")
        if r.get(f"{m.chat.id}:inMention:{Dev_dnyr}"):
            return False
        if r.get(f"{m.chat.id}:inMentionWAIT:{Dev_dnyr}"):
            get = r.ttl(f"{m.chat.id}:inMentionWAIT:{Dev_dnyr}")
            ...
            if len(text.split()) > 1:
                ...
                reason = ""
            users_list = []
            r.set(f"{m.chat.id}:inMention:{Dev_dnyr}", 1)
            ...
            for a in final_list:
                for i in a:
                    if not r.get(f"{m.chat.id}:inMention:{Dev_dnyr}"):
                        return False
                    ftext += f"{i} , "
                c.send_message(m.chat.id, ftext)
                ftext = f"{reason}\n\n"
            r.delete(f"{m.chat.id}:inMention:{Dev_dnyr}")
            r.set(f"{m.chat.id}:inMentionWAIT:{Dev_dnyr}", 1, ex=1200)
    ...
    if text == "تعطيل المنشن":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableALL:{Dev_dnyr}"):
                ...
                r.set(f"{m.chat.id}:disableALL:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت المنشن\n☆"
                )
    ...
    if text == "تفعيل المنشن":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableALL:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} المنشن مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableALL:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت المنشن\n☆"
                )
    ...
    if text == "تعطيل الترحيب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableWelcome:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الترحيب معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableWelcome:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الترحيب\n☆"
                )
    ...
    if text == "تعطيل الترحيب بالصورة" or text == "تعطيل الترحيب بالصوره":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableWelcomep:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الترحيب بالصورة من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableWelcomep:{Dev_dnyr}", 1)
                return m.reply(
    ...
    if text == "تفعيل الترحيب بالصورة" or text == "تفعيل الترحيب بالصوره":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableWelcomep:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الترحيب بالصورة مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableWelcomep:{Dev_dnyr}")
                return m.reply(
    ...
    if text == "تعطيل الرابط":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableLINK:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الرابط معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableLINK:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الرابط\n☆"
                )
    ...
    if text == "تفعيل الرابط":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableLINK:{Dev_dnyr}"):
                ...
                r.delete(f"{m.chat.id}:disableLINK:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت الرابط\n☆"
                )
    ...
    if text == "تعطيل البايو":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableBio:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} البايو معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableBio:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت البايو\n☆"
                )
    ...
    if text == "تفعيل البايو":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableBio:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} البايو مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableBio:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت البايو\n☆"
                )
    ...
    if text == "تعطيل اطردني":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:enableKickMe:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} اطردني معطل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:enableKickMe:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت اطردني\n☆"
                )
    ...
    if text == "تفعيل اطردني":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:enableKickMe:{Dev_dnyr}"):
                ...
                r.set(f"{m.chat.id}:enableKickMe:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت اطردني\n☆"
                )
    ...
    if text == "تعطيل التحقق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:enableVerify:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التحقق معطل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:enableVerify:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت التحقق\n☆"
                )
    ...
    if text == "تفعيل التحقق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:enableVerify:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التحقق مفعل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:enableVerify:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت التحقق\n☆"
                )
    ...
    if text == "تعطيل انطقي" or text == "تعطيل انطق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableSay:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} انطقي معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableSay:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت انطقي\n☆"
                )
    ...
    if text == "تفعيل انطقي" or text == "تفعيل انطق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableSay:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} انطقي مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableSay:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت انطقي\n☆"
                )
    ...
    if text.startswith("منع "):
        if mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.sismember(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}", noice):
                return m.reply(
                    f"{k} الكلمة ( {noice} ) مشجودة بقائمة المنع",
                    disable_web_page_preview=True,
                )
            else:
                r.sadd(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}", noice)
                return m.reply(
    ...
    if text.startswith("الغاء منع ") and len(text.split()) > 2:
        if mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.sismember(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}", noice):
                return m.reply(
                    f"{k} الكلمة ( {noice} ) مش مضافة بقائمة المنع",
                    disable_web_page_preview=True,
                )
            else:
                r.srem(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}", noice)
                return m.reply(
    ...
    if text == "منع" and m.reply_to_message and m.reply_to_message.media:
        if mod_pls(m.from_user.id, m.chat.id):
            ...
            id = file_id[-6:]
            if r.get(f"{id}:NotAllow:{m.chat.id}{Dev_dnyr}"):
                return m.reply(f"{k} مشجودة بقائمة المنع")
            else:
                r.set(f"{id}:NotAllow:{m.chat.id}{Dev_dnyr}", 1)
                r.sadd(
                    f"{m.chat.id}:NotAllowedList:{Dev_dnyr}",
    ...
    if text == "الغاء منع" and m.reply_to_message and m.reply_to_message.media:
        if mod_pls(m.from_user.id, m.chat.id):
            ...
            id = file_id[-6:]
            if not r.get(f"{id}:NotAllow:{m.chat.id}{Dev_dnyr}"):
                return m.reply(f"{k} مش مشجودة بقائمة المنع")
            else:
                r.delete(f"{id}:NotAllow:{m.chat.id}{Dev_dnyr}")
                r.srem(
                    f"{m.chat.id}:NotAllowedList:{Dev_dnyr}",
                    f"file={id}&by={m.from_user.id}&type={type}&file_id={file_id}",
                )
    ...
    if text == "قائمه المنع" or text == "قائمة المنع":
        ...
        if mod_pls(m.from_user.id, m.chat.id):
            if not r.smembers(
                f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"
            ) and not r.smembers(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}"):
                return m.reply(f"{k} مفيش شي ممنوع")
            else:
                if not r.smembers(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"):
                    text1 += "لايوجد"
                else:
                    for a in r.smembers(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"):
                        text1 += f"{count} - {a}\n"
                        count += 1
                if not r.smembers(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}"):
                    text2 += "لايوجد"
                else:
                    for a in r.smembers(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}"):
                        g = a
    ...
    if text == "مسح قائمه المنع" or text == "مسح قائمة المنع":
        if mod_pls(m.from_user.id, m.chat.id):
            if not r.smembers(
                ...
                if r.smembers(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}"):
                    r.delete(f"{m.chat.id}:NotAllowedListText:{Dev_dnyr}")
                if r.smembers(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}"):
                    for a in r.smembers(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}"):
                        file_id = a.split("file=")[1].split("&by=")[0]
                        r.delete(f"{file_id}:NotAllow:{m.chat.id}{Dev_dnyr}")
                r.delete(f"{m.chat.id}:NotAllowedList:{Dev_dnyr}")
    ...
    if text == "قفل الكل":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if (
                r.get(f"{m.chat.id}:mute:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockEdit:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockEditM:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockUrls:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockHashtags:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockTags:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockSpam:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}")
                and r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}")
            ):
                return m.reply(
                    f"{k} من 「 {m.from_user.mention} 」 \n{k} كل شي مقفل يالطيب!\n☆"
                )
                ...
                r.set(f"{m.chat.id}:mute:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockJoin:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockChannels:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockEdit:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockEditM:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockVoice:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockVideo:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockNot:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockPhoto:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockStickers:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockAnimations:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockFiles:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockPersian:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockUrls:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockHashtags:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockMessages:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockTags:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockBots:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockSpam:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockInline:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockForward:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockAudios:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockSHTM:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockNSFW:{Dev_dnyr}", 1)
                return False
    ...
    if text == "فتح الكل":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if (
                not r.get(f"{m.chat.id}:mute:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockEdit:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockEditM:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockUrls:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockHashtags:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockTags:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockSpam:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}")
                and not r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}")
            ):
                return m.reply(
                ...
                m.reply(f"{k} من 「 {m.from_user.mention} 」 \n{k} ابشر فتحت كل شي\n☆")
                r.delete(f"{m.chat.id}:mute:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockJoin:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockChannels:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockEdit:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockEditM:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockVoice:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockVideo:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockNot:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockPhoto:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockStickers:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockAnimations:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockFiles:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockPersian:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockUrls:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockHashtags:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockMessages:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockTags:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockBots:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockSpam:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockInline:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockForward:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockAudios:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockSHTM:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockKFR:{Dev_dnyr}")
                r.delete(f"{m.chat.id}:lockNSFW:{Dev_dnyr}")
                return False
    ...
    if text == "تفعيل الحماية" or text == "تفعيل الحمايه":
        if not owner_pls(m.from_user.id, m.chat.id):
            ...
            if (
                ...
                r.set(f"{m.chat.id}:lockChannels:{Dev_dnyr}", 1)
                r.delete(f"{m.chat.id}:disableWarn:{Dev_dnyr}")
                r.set(f"{m.chat.id}:lockVoice:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockVideo:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockPhoto:{Dev_dnyr}", 1)
                r.set(f"{m.chat.id}:lockStickers:{Dev_dnyr}", 1)
    ...
    if text == "قفل الدردشة" or text == "قفل الدردشه" or text == "قفل الشات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:mute:{Dev_dnyr}"):
    ...
    if text == "فتح الدردشة" or text == "فتح الدردشه" or text == "فتح الشات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:mute:{Dev_dnyr}"):
    ...
    if text == "قفل التعديل":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockEdit:{Dev_dnyr}"):
    ...
    if text == "فتح التعديل":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockEdit:{Dev_dnyr}"):
    ...
    if text == "قفل تعديل الميديا":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockEditM:{Dev_dnyr}"):
    ...
    if text == "فتح تعديل الميديا":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockEditM:{Dev_dnyr}"):
    ...
    if text == "قفل الفويسات" or text == "قفل البصمات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}"):
    ...
    if text == "فتح الفويسات" or text == "فتح البصمات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockVoice:{Dev_dnyr}"):
    ...
    if text == "قفل الفيديو" or text == "قفل الفيديوهات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}"):
    ...
    if text == "فتح الفيديو" or text == "فتح الفيديوهات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockVideo:{Dev_dnyr}"):
    ...
    if text == "قفل الاشعارات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}"):
    ...
    if text == "فتح الاشعارات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockNot:{Dev_dnyr}"):
    ...
    if text == "قفل الصور":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}"):
    ...
    if text == "فتح الصور":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockPhoto:{Dev_dnyr}"):
    ...
    if text == "قفل الملصقات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}"):
    ...
    if text == "فتح الملصقات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockStickers:{Dev_dnyr}"):
    ...
    if text == "قفل الفارسيه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}"):
    ...
    if text == "فتح الفارسيه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockPersian:{Dev_dnyr}"):
    ...
    if text == "قفل الملفات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}"):
    ...
    if text == "فتح الملفات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockFiles:{Dev_dnyr}"):
    ...
    if text == "قفل المتحركات" or text == "قفل المتحركه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}"):
    ...
    if text == "فتح المتحركات" or text == "فتح المتحركه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockAnimations:{Dev_dnyr}"):
    ...
    if text == "قفل الروابط":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockUrls:{Dev_dnyr}"):
    ...
    if text == "فتح الروابط":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockUrls:{Dev_dnyr}"):
    ...
    if text == "قفل الهشتاق" or text == "قفل الهاشتاق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockHashtags:{Dev_dnyr}"):
    ...
    if text == "فتح الهشتاق" or text == "فتح الهاشتاق":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockHashtags:{Dev_dnyr}"):
    ...
    if text == "قفل البوتات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}"):
    ...
    if text == "فتح البوتات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockBots:{Dev_dnyr}"):
    ...
    if text == "قفل اليوزرات" or text == "قفل المنشن":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockTags:{Dev_dnyr}"):
    ...
    if text == "فتح اليوزرات" or text == "فتح المنشن":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockTags:{Dev_dnyr}"):
   ...
   if text == 'قفل الكفر' or text == 'قفل الشيعه' or text == 'قفل الشيعة':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if r.get(f'{m.chat.id}:lockKFR:{Dev_dnyr}'):
         return m.reply(locknn.format(k,m.from_user.mention,k,'الكفر'))
       else:
         r.set(f'{m.chat.id}:lockKFR:{Dev_dnyr}',1)
   ...
   if text == 'فتح الكفر' or text == 'فتح الشيعه' or text == 'فتح الشيعة':
     if not admin_pls(m.from_user.id,m.chat.id):
       ...
       if not r.get(f'{m.chat.id}:lockKFR:{Dev_dnyr}'):
   ...
   """
    ...
    if text == "قفل الإباحي" or text == "قفل الاباحي":
        if not owner_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}"):
    ...
    if text == "فتح الإباحي" or text == "فتح الاباحي":
        if not owner_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockNSFW:{Dev_dnyr}"):
    ...
    if text == "قفل الكلام الكثير" or text == "قفل الكلايش":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}"):
    ...
    if text == "فتح الكلام الكثير" or text == "فتح الكلايش":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockMessages:{Dev_dnyr}"):
    ...
    if text == "فتح التكرار":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockSpam:{Dev_dnyr}"):
    ...
    if text == "قفل التوجيه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}"):
    ...
    if text == "فتح التوجيه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockForward:{Dev_dnyr}"):
    ...
    if text == "قفل الانلاين":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}"):
    ...
    if text == "فتح الانلاين":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockInline:{Dev_dnyr}"):
    ...
    if text == "قفل السب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}"):
    ...
    if text == "فتح السب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockSHTM:{Dev_dnyr}"):
    ...
    if text == "قفل الاضافه" or text == "قفل الاضافة" or text == "قفل الجهات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}"):
    ...
    if text == "فتح الاضافه" or text == "فتح الاضافة" or text == "فتح الجهات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockaddContacts:{Dev_dnyr}"):
    ...
    if text == "قفل دخول البوتات" or text == "قفل الوهمي" or text == "قفل الايراني":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}"):
                ...
                r.set(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}", 1)
    ...
    if text == "فتح دخول البوتات" or text == "فتح الوهمي" or text == "فتح الايراني":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}"):
                return m.reply(Openn2.format(k, m.from_user.mention, k, "دخول البوتات"))
            else:
                r.delete(f"{m.chat.id}:lockJoinPersian:{Dev_dnyr}")
    ...
    if text == "قفل الصوت":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}"):
    ...
    if text == "فتح الصوت":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockAudios:{Dev_dnyr}"):
    ...
    if text == "قفل القنوات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}"):
    ...
    if text == "فتح القنوات":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockChannels:{Dev_dnyr}"):
    ...
    if text == "قفل الدخول":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}"):
    ...
    if text == "فتح الدخول":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:lockJoin:{Dev_dnyr}"):
    ...
    if text == "تعطيل التحذير":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableWarn:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التحذير معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableWarn:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت التحذير\n☆"
                )
    ...
    if text == "تعطيل اليوتيوب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableYT:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} اليوتيوب معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableYT:{Dev_dnyr}", 1)
                return m.reply(
    ...
    if text == "تفعيل اليوتيوب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableYT:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} اليوتيوب مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableYT:{Dev_dnyr}")
                return m.reply(
    ...
    if text == "تعطيل الساوند":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableSound:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الساوند معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableSound:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الساوند\n☆"
                )
    ...
    if text == "تفعيل الساوند":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableSound:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الساوند مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableSound:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت الساوند\n☆"
                )
    ...
    if text == "تعطيل الانستا":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableINSTA:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الانستا معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableINSTA:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الانستا\n☆"
                )
    ...
    if text == "تفعيل الانستا":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableINSTA:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الانستا مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableINSTA:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت الانستا\n☆"
                )
    ...
    if text == "تعطيل اهمس":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableWHISPER:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} اهمس معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableWHISPER:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت اهمس\n☆"
                )
    ...
    if text == "تفعيل اهمس":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableWHISPER:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} اهمس مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableWHISPER:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت اهمس\n☆"
                )
    ...
    if text == "تعطيل التيك":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableTik:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التيك معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableTik:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت التيك\n☆"
                )
    ...
    if text == "تفعيل التيك":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableTik:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التيك مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableTik:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت التيك\n☆"
                )
    ...
    if text == "تعطيل شازام":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableShazam:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} شازام معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableShazam:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت شازام\n☆"
                )
    ...
    if text == "تفعيل شازام":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableShazam:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} شازام مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableShazam:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت شازام\n☆"
                )
    ...
    if text == "تعطيل الالعاب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableGames:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الالعاب معطله من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableGames:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الالعاب\n☆"
                )
    ...
    if text == "تفعيل الالعاب":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableGames:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الالعاب مفعله من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableGames:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت الالعاب\n☆"
                )
    ...
    if text == "تعطيل الترجمة" or text == "تعطيل الترجمه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableTrans:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الترجمه معطله من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableTrans:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت الترجمه\n☆"
                )
    ...
    if text == "تفعيل الترجمة" or text == "تفعيل الترجمه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableTrans:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الترجمه مفعله من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableTrans:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت الترجمه\n☆"
                )
    ...
    if text == "تعطيل التسلية" or text == "تعطيل التسليه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableFun:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التسلية معطله من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableFun:{Dev_dnyr}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت التسلية\n☆"
                )
    ...
    if text == "تفعيل التسلية" or text == "تفعيل التسليه":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableFun:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التسلية مفعله من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableFun:{Dev_dnyr}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت التسلية\n☆"
                )
    ...
    if text == "تعطيل الاشتراك":
        if not dev2_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"disableSubscribe:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الاشتراك الاجباري معطل من قبل\n☆"
                )
            else:
                r.set(f"disableSubscribe:{Dev_dnyr}", 1)
                return m.reply(
    ...
    if text == "قناة الاشتراك":
        if not dev2_pls(m.from_user.id, m.chat.id):
            return m.reply(f"{k} هذا الامر يخص ( المطور وفوق ) بس")
        ch = r.get(f"forceChannel:{Dev_dnyr}") or "مفيش قناة"
    ...
    if text.startswith("وضع قناة @"):
        ...
        r.set(f"forceChannel:{Dev_dnyr}", "@" + username)
    ...
    if text == "تفعيل الاشتراك":
        if not dev2_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"disableSubscribe:{Dev_dnyr}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} الاشتراك الاجباري مفعل من قبل\n☆"
                )
            else:
                r.delete(f"disableSubscribe:{Dev_dnyr}")
                return m.reply(
    ...
    if text.startswith("تقييد ") and len(text.split()) == 2:
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            try:
                ...
                return m.reply(f"{k} مفيش عضو بهذا اليوزر")
            c.restrict_chat_member(
    ...
    if text == "اهمس" and m.reply_to_message and m.reply_to_message.from_user:
        ...
        if user_id == m.from_user.id:
            return m.reply(f"{k} مفيشك تهمس لنفسك ياغبي")
        else:
            import uuid
    ...
    if text == "تعطيل التنظيف":
        if not gowner_pls(m.from_user.id, m.chat.id):
            ...
            if not r.hget(Dev_dnyr + str(m.chat.id), "ena-clean"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التنظيف معطل من قبل\n☆"
                )
            else:
                r.hdel(Dev_dnyr + str(m.chat.id), "ena-clean")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت التنظيف\n☆"
                )
    ...
    if text == "تفعيل التنظيف":
        if not gowner_pls(m.from_user.id, m.chat.id):
            ...
            if r.hget(Dev_dnyr + str(m.chat.id), "ena-clean"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} التنظيف مفعل من قبل\n☆"
                )
            else:
                r.hset(Dev_dnyr + str(m.chat.id), "ena-clean", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت التنظيف\n☆"
                )
    ...
    if re.search("^وضع وقت التنظيف [0-9]+$", text):
        if not gowner_pls(m.from_user.id, m.chat.id):
            ...
            if secs > 3600 or secs < 60:
                ...
                r.hset(Dev_dnyr + str(m.chat.id), "clean-secs", secs)
    ...
    if text == "وقت التنظيف":
        if not gowner_pls(m.from_user.id, m.chat.id):
            ...
            secs = r.hget(Dev_dnyr + str(m.chat.id), "clean-secs") or "60"
    ...
    if text == "طرد" and m.reply_to_message and m.reply_to_message.from_user:
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            try:
                ...
                return m.reply(f"{k} العضو مش بالمجمشعة")
        ...
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            try:
                ...
                if get.status == ChatMemberStatus.RESTRICTED:
                    ...
                    text += "تقييد\n"
                    co += 1
                if r.get(f"{get.user.id}:mute:{m.chat.id}{Dev_dnyr}"):
                    r.delete(f"{get.user.id}:mute:{m.chat.id}{Dev_dnyr}")
                    r.srem(f"{m.chat.id}:listMUTE:{Dev_dnyr}", get.user.id)
                    text += "كتم\n"
                    co += 1
                if co > 0:
    ...
    if text == "تنزيل مشرف" and m.reply_to_message and m.reply_to_message.from_user:
        if not owner_pls(m.from_user.id, m.chat.id):
            ...
            try:
                ...
                return m.reply(
                    f"「 {m.reply_to_message.from_user.mention} 」\n{k} مش انا الي رفعته او ماعندي صلاحيات"
                )
    ...
    if text == "رفع مشرف" and m.reply_to_message and m.reply_to_message.from_user:
        if not owner_pls(m.from_user.id, m.chat.id):
            ...
            if (
                ...
                return m.reply(
                    """
⇜ تمام دلوقت ارسل صلاحيات المشرف
...
☆""",
    ...
    if r.get(f"{m.from_user.id}:promote:{m.chat.id}") and owner_pls(
        ...
        if text.startswith("*"):
            ...
            if type == 1:
                r.set(f"{m.chat.id}:rankADMIN:{get.user.id}{Dev_dnyr}", 1)
                r.sadd(f"{m.chat.id}:listADMIN:{Dev_dnyr}", get.user.id)
                return m.reply(
    ...
    if (
        ...
        if admin_pls(m.from_user.id, m.chat.id):
            channel = (
                r.get(f"{Dev_dnyr}:BotChannel")
                if r.get(f"{Dev_dnyr}:BotChannel")
                else "YQYQY6"
            )
            return m.reply(
...
⌯ مسح الكل  ↢ بدون رد  ↢ لتنزيل كل رتب المجمشعة
...
⌯ راتب ↢ يديك راتبك كل ٥ دقيقة

⌯ بخشيش ↢ يديك بخشيش كل ٥ دقايق

⌯ زرف ↢ تزرف فلوس اشخاص كل ٥ دقايق

⌯ كنز ↢ يديك كنز كل ١٠ دقايق

⌯ استثمار ↢ تستثمر بالمبلغ اللي تبيه مع نسبة ربح مضمشنه من ١٪؜ الى ١٥٪؜ ( او استثمار فلوسي )

⌯ حظ ↢ تلعبها بأي مبلغ ياتدبله ياتخسره انت وحظك ( او حظ فلوسي )

⌯ عجله ↢ تلعب عجله الحظ ولو تشابهو ال ٣ ايمشجيات تكسب من ١٠٠ الف لحد ٣٠٠ الف انت وحظك
...
""",
   ...
   """
    ...
    name = r.get(f"{Dev_dnyr}:BotName") if r.get(f"{Dev_dnyr}:BotName") else "رعد"
...
""",
    ...
    if m.data == f"gowner+{m.from_user.id}":
        if not gowner_pls(m.from_user.id, m.message.chat.id):
            ...
            r.hset(Dev_dnyr + f"locks-{m.message.chat.id}", command, 0)
            return m.edit_message_text(
    ...
    if m.data == f"owner+{m.from_user.id}":
        if not gowner_pls(m.from_user.id, m.message.chat.id):
            ...
            command = m.message.reply_to_message.text.split(None, 2)[2]
            r.hset(Dev_dnyr + f"locks-{m.message.chat.id}", command, 1)
            return m.edit_message_text(
    ...
    if m.data == f"mod+{m.from_user.id}":
        if not gowner_pls(m.from_user.id, m.message.chat.id):
            ...
            command = m.message.reply_to_message.text.split(None, 2)[2]
            r.hset(Dev_dnyr + f"locks-{m.message.chat.id}", command, 2)
            return m.edit_message_text(
    ...
    if m.data == f"admin+{m.from_user.id}":
        if not gowner_pls(m.from_user.id, m.message.chat.id):
            ...
            command = m.message.reply_to_message.text.split(None, 2)[2]
            r.hset(Dev_dnyr + f"locks-{m.message.chat.id}", command, 3)
            return m.edit_message_text(
    ...
    if m.data == f"pre+{m.from_user.id}":
        if not gowner_pls(m.from_user.id, m.message.chat.id):
            ...
            command = m.message.reply_to_message.text.split(None, 2)[2]
            r.hset(Dev_dnyr + f"locks-{m.message.chat.id}", command, 4)
            return m.edit_message_text(
